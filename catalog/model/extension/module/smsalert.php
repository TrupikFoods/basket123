...
class ModelExtensionModuleSmsAlert extends Model {
	...
	public function getTemplates($type, $store_id,$customer_notify=1) { 
	...
	public function parseSMS($type, $store_id, $number, $replace,$vendor_phone='') {
		$number = preg_replace('/[^0-9]/', '', $number);
		$templates = $this->getTemplates($type, $store_id,1);
		$template_data = false;
		foreach ($templates as $template) {
				$template_data = $template;
		...
		if ($template_data && !empty($template_data) && $template_data['status']=='1') {
			...
			$admin_templates = $this->getTemplates($type, $store_id,0);
			$template_data = false;
