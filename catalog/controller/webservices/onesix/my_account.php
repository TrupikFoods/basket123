...
//                    if (!($this->customer->login($this->request->post['email']))) {
    ...
    public function appGetAddressForm() {
        ...
        $default_id = $this->config->get('config_country_id');
        $id = 0;
        if (!empty($this->request->post['email'])) {
            $this->load->model('account/customer');
            $customer_email = $this->request->post['email'];
        ...
        $data['default_state_id'] = isset($this->session->data['payment_address']['zone_id']) ? $this->session->data['payment_address']['zone_id'] : '';
